...
// ---------------------------------------------------------------------------
// sendError — classify, log 5xx, send structured JSON response
// ---------------------------------------------------------------------------
export function sendError(res, error, statusCode) {
  const classified = classifyError(error);

  // Caller may pass an explicit override (e.g., 404 for route not found)
  if (typeof statusCode === 'number' && statusCode !== classified.statusCode) {
    classified.statusCode = statusCode;
    classified.code = httpStatusCode(statusCode);
  }

  // Log all 5xx server errors with stack for observability
  if (classified.statusCode >= 500) {
    console.error(`[${new Date().toISOString()}] ${classified.statusCode} ${classified.code}:`, error);
  }
